...
def my_cum_simple(data, n):
    ret = np.zeros((n,n), dtype=float)
    
    for k in range(n):
        for l in range(n):
            ret[k, l] = np.mean(data*np.roll(data, k)*np.roll(data, l))

    return ret    
#    X, Y = np.meshgrid(range(n), range(n))
#    fig = plt.figure()
#    ax = fig.gca(projection='3d')
#    ax.plot_surface(X, Y, ret, label="blub")
#    plt.show()
...
#    data = np.zeros((3,100))
#    data[0,:] = np.random.rayleigh(1, n)
#    data[1, :] = np.roll(data[0,:], 1)
#    data[2, :] = np.roll(data[0,:], 2)
    
#    my_cum_simple(data, n)
    ret = my_cum_simple(data, n)
    X, Y = np.meshgrid(range(n), range(n))
    fig = plt.figure()
    ax = fig.gca(projection='3d')
    ax.plot_surface(X, Y, ret, label="blub")
